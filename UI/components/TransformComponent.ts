export default class TransformComponent {
    ...
    public transform(): HTMLElement {
        ...
        const transformedDoc = xsltProcessor.transformToFragment(
            xmlDoc,
            document,
        );
        ...
        if (!transformedDoc) {
            console.error(
                'XSLT Transformation Failed. XML or XSLT might be invalid.',
            );
            throw new Error(
                'XSLT transformation failed. Please check the XML and XSLT inputs.',
            );
        }
        ...
        if (!questionElement) {
            console.error(
                'The transformed document does not contain a <question> root element.',
            );
            throw new Error(
                'The transformed document does not contain a <question> root element.',
            );
