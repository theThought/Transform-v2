...
- All stories and docs are ordered according to the `storySort` configuration in `.storybook/preview.ts`.

## CSS and JavaScript
- The compiled CSS/JavaScript code is loaded inside `./UI/.storybook/preview-head.html`.
- Storybook's `public` folder defaults to `staticDirs: ['../public']` folder inside `./UI/.storybook/main.ts`.
